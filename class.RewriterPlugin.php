...
class RewriterPlugin extends Plugin
{
    ...
    private function process_ticket(&$vars)
    {
        ...
        if (strlen($restrict_forwarding_to_these_domains)) {
            ...
            $regex_of_allowed_domains = '/.+@(' . str_replace(',', '|', $restrict_forwarding_to_these_domains) . ')/i';
        ...
        if (preg_match('/^\[?Fwd?: /i', $subject) || preg_match('/\(fwd\)$/i', $subject)) {
     ...
     *
     * We convert the HTML of the message into a DOMDocument, then iterate through all the nodes that
     * make it up.
     *
     * In each of the nodes, we search the text of the node looking for email addresses, because,
     * From: Name <email@address.org> will appear at some point as a piece of text.
     *
     * When we've found the email address, we look through the lines that make up that node's text
     * and find the From: part, looking backwards.
     *
     * When we find the From: line, we trim that off, and trim off the email address, and we're just left with the name.
     *
     * @param unknown $html
