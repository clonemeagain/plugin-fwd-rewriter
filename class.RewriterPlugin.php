...
/**
 * The goal of this Plugin is to modify tickets as they are being created from email.
 * 
 * Allows Rewriting or Redirecting from the forwarding user to the actual Owner 
 * For instance: using a webform or other to create your tickets via email, the
...
class RewriterPlugin extends Plugin {
    ...
    public function bootstrap() {
        ...
        // See if admin wants to prune metadata from responses (different signal)
        if ($this->getConfig()->get('strip-metadata')) {
            Signal::connect('threadentry.created', function(ThreadEntry $entry) {
                $this->stripMetadata($entry);
            });
        }
        ...
        if ($this->getConfig()->get('delete-attachments')) {
            ...
            // apparently not.
            Signal::connect('mail.processed', function ($mf, &$vars) {
    ...
    private function process_ticket(&$vars) {
        ...
        if ($this->getConfig()->get('drupal') &&
            ...
            if (preg_match_all(
                            '#([\w ]+) \((.*)\) sent a message using the contact form at#i', $message_body, $sender)) {
...
// note the second ?, because Office365 forwards with "Fw: subject".. special.
    ...
    private function optimisticSearch($message_body) {
        $sender = array();
        if (preg_match_all(
                        '/From:(.*)(?: |<|;|>)([\w\d_\-\.]+@[\w\d_\-\.]+)(?:&|>| )/i', $matchable_body, $sender)) {
            if (self::DUMPWHOLETHING) {
