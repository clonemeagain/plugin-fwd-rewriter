        ...
        if ($config['email-rewrite']) {
            ...
            foreach ($rules as $rule) {
                if (! $rule)
                    continue;
                list ($pattern, $replacement) = explode(':', $rule);
                Messages::success("Email string $pattern will be replaced with $replacement");
            }
        }
        return TRUE;
    }
    ...
    function getOptions()
    {
        ...
        return array(
            ...
            'sba' => new SectionBreakField(array(
                'label' => $__('Find & Replace: one per line, seperate split with : case insensitive'),
                'hint' => $__('A missing second value indicates delete the match.')
            )),
            'email-rewrite' => new TextareaField(array(
                'label' => $__('Arbitrary Rewrite email Addresses'),
                'hint' => $__('Use find:replace pairs, like: @internal.local:@company.com'),
                'configuration' => array(
                    'html' => FALSE
                )
            )),
            'text-rewrite' => new TextareaField(array(
            ...
            'sbr' => new SectionBreakField(array(
            ...
            'regex-rewrite' => new TextareaField(array(
                'configuration' => array(
                    'html' => FALSE
                ),
